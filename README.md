**Zomato UX Case Study** 
Redesigning User Experience for Better Clarity, Faster Decision-Making & Higher Satisfaction 

This repository contains my UX Case Study on Zomato — one of India’s leading food delivery platforms.  

The study focuses on identifying usability issues and proposing design improvements supported by sketches, mockups, and UX reasoning.

 ****Project Overview**** 
Zomato is widely used for food ordering and restaurant discovery, but certain parts of the user journey still contain friction.  

This case study aims to improve:

  >Discoverability of deals & restaurant info  
  >Faster decision-making  
  >Quicker menu navigation  
  >Transparent coupon visibility  
  >Reduced cognitive load  
  >Overall user satisfaction  

****User Journey Breakdown****  
1. **App Launch & Onboarding**  
2. **Home Screen Browsing**  
3. **Restaurant Discovery & Comparison**  
4. **Menu Navigation**  
5. **Item Customization & Cart Review**  
6. **Order Tracking**  
7. **Post-Order Feedback**

****Key Strengths Identified****  
1.Strong visual design 
2.Personalization using recommendations
3.Powerful tracking system
4.Effective filtering & sorting  

****Problems Identified**** 
1.Cluttered homepage  
2.Discounts visible only late in checkout  
3.Long scrolling menus  
4.Insufficient dish images
5.High cognitive load while comparing  

---

## 🎨 Proposed UX Improvements  
### ✔ 1. Clean & Priority-Based Home Screen  
- Reduced clutter  
- Clear hierarchy  
- Simplified browsing  

### ✔ 2. Transparent Deal Visibility  
- Deals shown earlier on listing cards  
- Color-coded badges  
- Increased trust & clarity  

### ✔ 3. Smart Menu Navigator  
- Sticky side navigation  
- Category jump shortcuts  
- Faster food discovery  

### ✔ 4. Enhanced Review Summaries  
- AI-based review highlights  
- Trend analysis  
- Time saved for users  

### ✔ 5. Predictive Add-Ons  
- Intelligent upsells  
- Smart suggestions based on order history  

---

****Included Deliverables****  
1.This repo includes:
2.Final UX Case Study PDF
3.Hand-drawn sketches & mockups  
4.Redesigned UI frames  
5.UX problem–solution mapping  

 ****Tools Used**** 
1.Figma  
2.Adobe XD (optional) 
3.Google Docs / Word  
4.GitHub  



****Author****  
**Pushpasri Venkatesh**  
CSE Student | UX Enthusiast | Learning by building  


****Contact****  
For suggestions 
Email: pushpasrivenkat2007@gmail.com

